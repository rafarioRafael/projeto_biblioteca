...
exports.atribuirLivro = void 0;
const connection_1 = __importDefault(require("../db/connection"));
const atribuirLivro = (req, res) => {
    const { idLivro, idPessoa } = req.params;
    // Primeiro, você pode verificar se há estoque disponível antes de atribuir o livro
    connection_1.default.query('SELECT qtd FROM tbl_estoque WHERE id_livro = ?', idLivro, (err, result) => {
        if (err) {
        ...
        connection_1.default.query('INSERT INTO cliente_livro (id_clientes, id_livros,is_active) VALUES (?, ?, ?)', [idPessoa, idLivro, true], (err, data) => {
            if (err) {
                return res.status(500).json({ error: 'Erro ao atribuir o livro' });
            }
            // Agora você pode atualizar a quantidade no estoque
            connection_1.default.query('UPDATE tbl_estoque SET quantidade = quantidade - 1 WHERE id_livro = ?', idLivro, (err, data) => {
                if (err) {
...
exports.atribuirLivro = atribuirLivro;
